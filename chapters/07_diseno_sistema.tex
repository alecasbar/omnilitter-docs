...
En el prototipo inicial del portal web, \texttt{localStorage} se utiliza como mecanismo de
persistencia de la sesión del usuario entre recargas de página, almacenando el estado
necesario para simular autenticación, rol y grupo activo. En la implementación integrada,
este comportamiento se vincula con los \textit{tokens} emitidos por el backend y con una
estrategia de refresco de sesión.

En el prototipo inicial, la comunicación entre el selector de grupo activo (ubicado en la
barra lateral) y el componente Dashboard se resolvía mediante un evento personalizado
del DOM (\texttt{omnilitter:groupChanged}). Esta solución es suficiente para el
prototipo; en una evolución posterior podría sustituirse por un contexto global o
una librería de estado si la complejidad de la interfaz aumentara.
